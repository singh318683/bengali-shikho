"""
Generates the Listen/Slow audio for every phrase in Bengali Shikho, using
...
Run:
    ...
    python make_audio.py --list
        Prints every Bengali (bn-IN) voice edge-tts currently has, in case
        the default voice below stops working or you want to try the other
        one.

    python make_audio.py --voice bn-IN-BashkarNeural
...
Reads its word list from data/items.json (kept in sync with data/bengali.js).
"""
import argparse
import asyncio
import json
...
DEFAULT_VOICE = "bn-IN-TanishaaNeural"  # female. Alternative: bn-IN-BashkarNeural (male)
...
async def list_voices():
    ...
    if not ta_voices:
        print("No bn-IN voices were returned by edge-tts right now.")
        print("Full locale list sample:", sorted({v['Locale'] for v in voices})[:20], "...")
        return
    print("Bengali (bn-IN) voices available:")
    for v in ta_voices:
...
async def generate(voice: str):
    if not ITEMS_PATH.exists():
        sys.exit(f"Missing {ITEMS_PATH}. Run this script from the bengali-shikho folder.")
...
def main():
    ...
    parser.add_argument("--list", action="store_true", help="list available bn-IN voices and exit")
